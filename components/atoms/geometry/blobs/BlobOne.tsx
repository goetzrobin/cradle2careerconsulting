...
const BlobOne = ({ color = Colors.primary, hasImage = false, image= '/_next/static/images/dooo.jpg',
 width = '100%', height = '100%' }) => {
    ...
    let placeholderPath = require(`../../../../content/${image}?lqip`);
    let imagePath = require(`../../../../content/${image}?url`);
    const handleOnLoad = () => {
        setLoaded(true);
    }
    useEffect(() => loaded && imagePath && setLoaded(true))
    return <svg width={width} height={height} viewBox="0 0 604 484" fill="none" xmlnsXlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg">
        ...
        {hasImage && <defs>
            <pattern id="pattern123" patternContentUnits="objectBoundingBox" width="1" height="1">
                <use xlinkHref={loaded ? '#image123' : '#image134'} transform="translate(-0.0650376) scale(0.00101809 0.00135135)" />
            </pattern>
            <image id="image134" width="1200" height="740" href={placeholderPath}/>
            <image onLoad={handleOnLoad} id="image123" width="1200" height="740" href={imagePath}/>
        </defs>
        }
    </svg>;
}
